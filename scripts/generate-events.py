...
activityTemplate = [["Pay", "Request", "Split" ], ["PhonePicked", "VpaPicked", "Cancelled"], ["Wallet", "Account"], ["Finished", "Error"]]

categoryNames = ["Electronics", "Media", "Fashion"]
cityNames = ["Kolkata", "Bangalore", "Delhi", "Chennai", "Mumbai"]

eventNames = ['LOGIN', 'LOGIN', 'LOGIN', 'LOGIN', 'LOGIN', 'LOGIN', 'PAY', 'PAY', 'PAY', 'PAY','PAY',  'REQUEST', 'REQUEST', 'LOGOUT']
os = [['android', 'ics'], ['android', 'jellybean'], ['android', 'lollypop'], ['android', 'lollypop'], ['android', 'kitkat'], ['android', 'kitkat'], ['android', 'kitkat'], ['android', 'kitkat'], ['android', 'kitkat'], ['android', 'marshmallow'], ['android', 'marshmallow'], ['android', 'marshmallow'], ['ios', '8'], ['ios', '9'], ['ios', '9'],['ios', '9']]
...
def postBatch(args):
  events = eventBatch()
  r = requests.post(url="http://" + args.server + "/foxtrot/v1/document/test/bulk", data=json.dumps(events), headers={'Content-type': 'application/json'})
  print r
