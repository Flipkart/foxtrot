...
public class Auth {
    ...
    @Inject
    public Auth(Provider<AuthStore> authProvider,
    ...
    public Response provisionUser(@NotNull @Valid final CreateUserRequest createUserRequest) {
        ...
        return Response.ok(authProvider.get()
                .provisionUser(user))
                .build();
    }
    ...
    public Response deleteUser(@NotNull @NotEmpty @PathParam("userId") final String userId) {
        final boolean status = authProvider.get()
                .deleteUser(userId);
        if (!status) {
    ...
    public Response provisionToken(@NotNull @NotEmpty @PathParam("userId") final String userId) {
        val token = authProvider.get()
                .provisionToken(userId, TokenType.STATIC, null)
                .orElse(null);
        if (null == token) {
            return Response.notModified()
                    .build();
        }
        return Response.ok(Collections.singletonMap("jwt", AuthUtils.createJWT(token, authConfig.getJwt())))
                .build();
    }
    ...
    private Response updateUserResponse(String userId,
                                        boolean status) {
        if (!status) {
            return Response.notModified()
                    .build();
