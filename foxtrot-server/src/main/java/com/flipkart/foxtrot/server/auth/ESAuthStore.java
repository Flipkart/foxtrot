...
public class ESAuthStore implements AuthStore {
    ...
    @Inject
    public ESAuthStore(ElasticsearchRestConnection connection,
                       ObjectMapper mapper) {
    ...
    public boolean deleteUser(String id) {
        boolean status = connection.getClient()
                .delete(new DeleteRequest(USERS_INDEX, USER_TYPE, id).setRefreshPolicy(
        ...
        try {
            val saveStatus = connection.getClient()
                    .index(new IndexRequest(TOKENS_INDEX).source(
                            mapper.writeValueAsString(new Token(tokenId, tokenType, userId, expiry)), XContentType.JSON)
                            .id(tokenId)
                            .type(TOKEN_TYPE)
                            .create(true)
    ...
    public Optional<Token> getTokenForUser(String userId) {
        val getResp = connection.getClient()
                .search(new SearchRequest(TOKENS_INDEX).searchType(SearchType.QUERY_THEN_FETCH)
                                .types(TOKEN_TYPE)
        ...
        return Optional.of(mapper.readValue(hits.getAt(0)
                .getSourceAsString(), Token.class));
    }
    ...
    public boolean deleteToken(String tokenId) {
        return connection.getClient()
                .delete(new DeleteRequest(TOKENS_INDEX, TOKEN_TYPE, tokenId).setRefreshPolicy(
                        WriteRequest.RefreshPolicy.IMMEDIATE), RequestOptions.DEFAULT)
                .status() == RestStatus.OK;
    }
