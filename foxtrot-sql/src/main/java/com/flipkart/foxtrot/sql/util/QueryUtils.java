...
public class QueryUtils {
    ...
    public static String expressionToString(Expression expression) {
        if(expression instanceof Column) {
            return ((Column)expression).getFullyQualifiedName().replaceAll(Constants.REGEX, Constants.REPLACEMENT);
        }
        if(expression instanceof StringValue) {
            return ((StringValue)expression).getValue().replaceAll(Constants.REGEX, Constants.REPLACEMENT);
        }
        return null;
    }
