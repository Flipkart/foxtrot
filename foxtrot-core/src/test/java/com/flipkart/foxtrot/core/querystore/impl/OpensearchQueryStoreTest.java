 ...
 */
public class OpensearchQueryStoreTest {
    ...
    public void setUp() throws Exception {
        ...
        CardinalityConfig cardinalityConfig = new CardinalityConfig("true",
                String.valueOf(OpensearchUtils.DEFAULT_SUB_LIST_SIZE));
        ...
        this.tableMetadataManager = new DistributedTableMetadataManager(hazelcastConnection, opensearchConnection,
        ...
        this.queryStore = new OpensearchQueryStore(tableMetadataManager, opensearchConnection, dataStore, mutators,
                mapper, cardinalityConfig);
    }
    ...
    public void testSaveSingleRawKeyVersion1() throws Exception {
        ...
        GetResponse getResponse = opensearchConnection.getClient()
                .get(new GetRequest(
                                OpensearchUtils.getCurrentIndex(TestUtils.TEST_TABLE_NAME, originalDocument.getTimestamp()),
                                originalDocument.getId()).storedFields(OpensearchUtils.DOCUMENT_META_TIMESTAMP_FIELD_NAME),
    ...
    public void testSaveSingleRawKeyVersion2() throws Exception {
        ...
        GetResponse getResponse = opensearchConnection.getClient()
                .get(new GetRequest(
                                OpensearchUtils.getCurrentIndex(TestUtils.TEST_TABLE_NAME, originalDocument.getTimestamp()),
                                translatedDocument.getId()).storedFields(OpensearchUtils.DOCUMENT_META_TIMESTAMP_FIELD_NAME),
                        RequestOptions.DEFAULT);
        assertTrue("Id should exist in ES", getResponse.isExists());
    ...
    public void testSaveBulkLargeTextNodeWithBlockingEnabled() throws Exception {
        ...
        doReturn(translatedDocument).when(dataStore)
                .save(table, originalDocument);
        queryStore.save(TestUtils.TEST_TABLE_NAME, originalDocument);
        val currentIndex = OpensearchUtils.getCurrentIndex(TestUtils.TEST_TABLE_NAME, originalDocument.getTimestamp());
        val response = opensearchConnection.getClient()
                .get(new GetRequest(currentIndex, originalDocument.getId()).storedFields(
                                OpensearchUtils.DOCUMENT_META_TIMESTAMP_FIELD_NAME, "testField", "testLargeField"),
                        RequestOptions.DEFAULT);
        ...
        val mappings = opensearchConnection.getClient()
                .indices()
    ...
    public void testSaveBulkNestedLargeTextNode() throws Exception {
        ...
        String[] fields = {OpensearchUtils.DOCUMENT_META_TIMESTAMP_FIELD_NAME, "testField",
                String.format("testLargeField%s", StringUtils.repeat(".testField", 5))};
        val response = opensearchConnection.getClient()
                .get(new GetRequest(currentIndex, originalDocument.getId()).storedFields(fields),
                        RequestOptions.DEFAULT);
        assertTrue(response.isExists());
    ...
    public void testSaveBulkRawKeyVersion1() throws Exception {
        ...
        for (Document document : documents) {
            GetResponse getResponse = opensearchConnection.getClient()
                    .get(new GetRequest(
                                    OpensearchUtils.getCurrentIndex(TestUtils.TEST_TABLE_NAME, document.getTimestamp()),
                                    document.getId()).storedFields(OpensearchUtils.DOCUMENT_META_TIMESTAMP_FIELD_NAME),
                            RequestOptions.DEFAULT);
            assertTrue("Id should exist in ES", getResponse.isExists());
    ...
    public void testGetSingleRawKeyVersion1() throws Exception {
        ...
        opensearchConnection.refresh(OpensearchUtils.getIndices(TestUtils.TEST_TABLE_NAME));
    ...
    public void testEsClusterHealth() throws ExecutionException, InterruptedException, FoxtrotException {
        ...
        assertEquals("opensearch", clusterHealth.getClusterName());
    ...
    @Test
    public void testIndicesStats() throws FoxtrotException, ExecutionException, InterruptedException, IOException {
        List<Document> documents = Lists.newArrayList();
        for (int i = 0; i < 10; i++) {
            documents.add(createDummyDocument());
        }
    ...
    private Document createLargeNestedDocumentWithLargeTextNodeAsKey() {
        return Document.builder()
                ...
                .data(mapper.createObjectNode()
                        ...
                        .set("testLargeField",
                                createNestedObject("testField", StringUtils.repeat("*", 5000))))
                .build();
    }
    ...
    private Document createLargeNestedDocumentWithLargeTextNodeAsArrayValue() {
        return Document.builder()
                ...
                .data(mapper.createObjectNode()
                        .put("testField", "SINGLE_SAVE")
                        .set("testLargeField",
                                createNestedArray("testField", StringUtils.repeat("*", 5000))))
                .build();
    }
    ...
    private ObjectNode createNestedObject(String field, String value) {
        ...
        objectNode.with(field).with(field).with(field).put(field, value);
        return objectNode;
    }

    private ObjectNode createNestedArray(String field, String value) {
        ObjectNode objectNode = mapper.createObjectNode();
        objectNode.with(field).with(field).with(field).put(field, value).put(field + "_array", UUID.randomUUID().toString());
