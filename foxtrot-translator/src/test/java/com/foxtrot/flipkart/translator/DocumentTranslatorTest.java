...
import com.flipkart.foxtrot.pipeline.processors.string.StringLowerProcessor;
...
public class DocumentTranslatorTest {
    ...
    public void testPipelineExecution() throws Exception {
        TestUtils.setupJsonPath();
        val pipelineFetcher = mock(PipelineFetcher.class);
        when(pipelineFetcher.fetch(any())).thenReturn(new Pipeline(TEST_PIPELINE_NAME, ImmutableList.of(new StringLowerProcessorDefinition("$.name")), false, PipelineExecutionMode.SERIAL));
        Table table = Table.builder()
                .name("RANDOM_TABLE")
                .defaultPipeline(TEST_PIPELINE_NAME)
                .build();
        table.setDefaultPipeline(TEST_PIPELINE_NAME);
        TranslatorConfig translatorConfig = new TranslatorConfig();
        translatorConfig.setRawKeyVersion("2.0");
        translatorConfig.setUnmarshallerConfig(UnmarshallerConfig.builder()
                .unmarshallingEnabled(true)
        ...
        DocumentTranslator documentTranslator = new DocumentTranslator(translatorConfig, pipelineFetcher,
                new PipelineExecutor(
                        process -> new StringLowerProcessor(new StringLowerProcessorDefinition("$.name"))));

        Document originalDocument = new Document();
        originalDocument.setId(UUID.randomUUID()
                .toString());
        originalDocument.setTimestamp(System.currentTimeMillis());
        originalDocument.setData(mapper.createObjectNode()
                .put("name", "CAPS_NAME"));

        val translatedDocument = documentTranslator.translate(table, originalDocument);

        assertEquals("Name Should Be lower", "caps_name", translatedDocument.getData()
                .get("name")
                .asText());
    }

    @Test
    public void testPipelineExecutionDoesnotHappenIfPipelineExecutorisNull() throws Exception {
        TestUtils.setupJsonPath();
        val pipelineFetcher = mock(PipelineFetcher.class);
        when(pipelineFetcher.fetch(any())).thenReturn(new Pipeline(TEST_PIPELINE_NAME, ImmutableList.of(new StringLowerProcessorDefinition("$.name")), false, PipelineExecutionMode.SERIAL));
        Table table = Table.builder()
        ...
        assertEquals("Name Should Be Uppercase", "CAPS_NAME", translatedDocument.getData()
                .get("name")
                .asText());
    }
